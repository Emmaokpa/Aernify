...
function SubmissionList() {
  ...
  const handleDecision = async (submission: AffiliateSaleSubmission, decision: 'approve' | 'reject') => {
    ...
    const submissionRef = doc(firestore, 'affiliate_sale_submissions', submission.id);
    ...
    try {
      await batch.commit();
      toast({
        title: `Sale ${decision === 'approve' ? 'Approved' : 'Rejected'}`,
    ...
    return (
      <div className="text-center py-20 rounded-lg bg-card border">
        ...
        <p className="mt-2 text-muted-foreground">
          There are no pending affiliate sale submissions to review.
        </p>
      </div>
    );
  }
    ...
    <div className="grid sm:grid-cols-1 lg:grid-cols-2 gap-6">
      {submissions?.map((submission) => (
        <Card key={submission.id}>
          <CardHeader>
            <CardTitle>{submission.affiliateProductTitle}</CardTitle>
          ...
          <CardContent className="space-y-2">
            <p className='text-sm font-medium'>Proof of Sale (e.g., Transaction ID):</p>
            <Input readOnly value={submission.proofOfSale} />
...
export default function AdminVerifySalesPage() {
  return (
    <AdminAuthWrapper>
      <PageHeader
        title="Admin: Verify Affiliate Sales"
        description="Approve or reject user submissions for high-value affiliate sales."
      />
